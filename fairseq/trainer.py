...
class Trainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, quantizer=None):
        ...
        self._train_dummy_batch, self._valid_dummy_batch = None, None
    ...
    def get_train_iterator(
        ...
        self.reset_dummy_batch(batch_iterator.first_batch, "train")
    ...
    def get_valid_iterator(
        ...
        self.reset_dummy_batch(batch_iterator.first_batch, "valid")
        return batch_iterator
    ...
    def reset_dummy_batch(self, batch, split_type):
        #self._dummy_batch = batch
        if split_type == 'train':
            self._train_dummy_batch = batch
        elif split_type == 'valid':
            self._valid_dummy_batch = batch
        else:
            raise Exception('Unknown dummy type')
    ...
    def valid_step(self, sample, raise_oom=False):
        ...
        with torch.no_grad():
            ...
            sample, is_dummy_batch = self._prepare_sample(sample, split_type='valid')
    ...
    def _prepare_sample(self, sample, is_dummy=False, split_type="train"):
        if sample == "DUMMY":
            raise Exception(
        ...
        if sample is None or len(sample) == 0:
            ...
            if split_type == 'train':
                assert (
                    self._train_dummy_batch is not None and len(self._train_dummy_batch) > 0
                ), "Invalid dummy batch: {}".format(self._train_dummy_batch)
                sample, _ = self._prepare_sample(self._train_dummy_batch, is_dummy=True, split_type=split_type)

            elif split_type == 'valid':
                assert (
                    self._valid_dummy_batch is not None and len(self._valid_dummy_batch) > 0
                ), "Invalid dummy batch: {}".format(self._valid_dummy_batch)
                sample, _ = self._prepare_sample(self._valid_dummy_batch, is_dummy=True, split_type=split_type)
            return sample, True
        ...
        if split_type == 'train' and self._train_dummy_batch == "DUMMY": #this dummy is from reset_dummy function, means that can't get first_batch from iterator
            self._train_dummy_batch = sample
        if split_type == 'valid' and self._valid_dummy_batch == "DUMMY":
