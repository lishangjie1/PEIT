...
class Wav2VecModel(BaseFairseqModel):
    ...
    def __init__(self, cfg: Wav2VecConfig):
        ...
        if cfg.offset == "auto":
            jin = 0 # offset in receptive field, e.g. 10ms [0 ms,30 ms] [10ms 40ms]
            rin = 0 # receptive field, e.g. 30ms in raw wave
...
class Wav2VecPredictionsModel(nn.Module):
    ...
    def forward(self, x, y):
        ...
        steps = min(steps, tsz - self.offset) # tsz - self.offset (prevent overlap in receptive field)

        predictions = x.new(
            bsz * copies * (tsz - self.offset + 1) * steps
            - ((steps + 1) * steps // 2) * copies * bsz
        ) # last few features don't have steps future features. e.g. steps=12 and tsz=935 and offset=3 total predictions is (935-12-3+1)*12 + 11+10+...1
        if self.infonce:
