...
class ItransformerConfig(FairseqDataclass):
    multi_line: bool = field(
        default=False,
        metadata={
            "help": "multi line image translation ability"
        },
    )
    cnnfix: bool = field(
        default=False,
        metadata={
            "help": "fix the parameters of cnn encoder"
        },
    )
    embfix: bool = field(
        default=False,
    ...
    # pretrained ocr model as cnn encoder
    use_pretrained_ocr: bool = field(
        default=False,
        metadata={
            "help":"pretrained ocr model for cnn encoder"
        },
    )
    pretrained_ocr_path: str = field(
        default=None,
        metadata={
            "help":"model path of the pretrained ocr model for cnn encoder"
        },
    )
    # quantizer
    use_codebook: bool = field(
        default=False,
        metadata={
            "help":"codebook for the output of cnn encoder"
        },
    )
    codebook_prob: float = field(
        default=0.1,
        metadata={
            "help":"the probability of the use of codebook"
        },
    )
    
    vq_num_vars:int = field(
        default=320,
        metadata={"help":"num_vars of quantizer group"}
    )

    vq_temp:str = field(
        default="[2, 0.5, 0.999995]",
        metadata={"help":"temperature for training. this should be a tuple of 3 elements: (start, stop, decay factor)"}
    )

    vq_groups:int = field(
        default=2,
        metadata={"help":"number of groups for vector quantization"}
    )

    vq_quantizer_depth:int = field(
        default=1,
        metadata={"help":"number of layers (with activation in between) to project input before computing logits"}
    )

    vq_quantizer_factor:int = field(
        default=3,
        metadata={"help":"scales the inner dimensionality of projections by this factor"}
    )
